...
(defn get-moneyreport [request]
  (let[params (:params request)
       ...
       get-resultsumsql (str "SELECT * FROM(SELECT '' AS NAME,'' AS identityid,'' AS address,CONCAT('',count(*)) AS servicername,'' AS servicephone,'' AS serviceaddress,0 AS jja_id," sumcol ",'' AS servicetime,'' AS assesstype,'' AS districtid,SUM(subsidy_money) AS subsidy_money, '澉总计' AS dvname FROM ("
...
(defn get-yearmoneyreport [request]
  (let[params (:params request)
       ...
       get-mreportsql (apply str (unionsql sf ym))
       get-moneysql (str "select jja_id," col "from (" get-mreportsql ") group by jja_id")
       get-resultsql (str "select jm.name,jm.identityid,jm.address,jm.servicername,jm.servicephone,jm.serviceaddress, (case jm.economy   when '0' then '低保特困职工' when '1' then '低保边缘户' when '2' then '低收入' when '3' then '无退休工资' when '4' then '有退休工资' when '5' then '特殊贡献' else ''   end) as economy, s.*,jm.servicetime,jm.assesstype,jm.districtid,h.subsidy_money,dv.dvname  from ( " get-moneysql ") s
                           left join  (select j.jja_id,j.name,j.identityid,j.address,j.districtid,j.economy,a.servicername,a.servicephone,a.serviceaddress,a.servicetime,a.assesstype from t_jjylapply j
                                         left join   (select ds.servicername,ds.servicephone,ds.serviceaddress,t.jja_id,t.servicetime,t.assesstype from t_jjylassessment t
                                                 left join t_depservice ds on t.s_id = ds.s_id) a
                                         on j.jja_id = a.jja_id) jm
                           on s.jja_id = jm.jja_id
