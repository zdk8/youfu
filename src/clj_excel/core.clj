(ns clj-excel.core
  ...
  ;;(:import [org.apache.poi.xssf.usermodel XSSFWorkbook])”…”⁄œ¬‘ÿjar≤ªµΩ,‘›Õ£ π”√
...
(defn ^Workbook workbook-xssf
  ...
  ;;([] (new XSSFWorkbook)) ”…”⁄œ¬‘ÿpoi-ooxmlº∞“¿¿µjar≤ªµΩµƒ‘≠“Ú,‘›Õ£ π”√XSSFWorkbook,”…HSSFWorkbook¥˙ÃÊ
  ([] (new HSSFWorkbook))
...
(defn ^Workbook workbook-hssf
  "Create or open new excel workbook. Defaults to xls format.(ÂàõÂª∫ÊàñÊâìÂºÄÊñ∞ÁöÑExcelÂ∑•‰ΩúÁ∞ø„ÄÇÁº∫ÁúÅ‰∏∫XLSÊ†ºÂºè„ÄÇ)"
  ([] (new HSSFWorkbook))
  ([input ] (new HSSFWorkbook (input-stream input)))
  ;;([input] (WorkbookFactory/create (input-stream input)))
  )
...
(defn set-cell
  "Set cell at specified location with value.(ËÆæÁΩÆÂçïÂÖÉÊ†ºÂú®ÊåáÂÆö‰ΩçÁΩÆÁöÑÂÄº)"
...
(defn build-workbook
  "Build workbook from map of sheet names to multi dimensional seqs (ie a seq of seq)(‰ªéÂØπÂ∫îË°®ÂêçÁöÑmapÂª∫Á´ãÂ∑•‰ΩúËñÑÂ§ö‰ΩçÂ∫èÂàó)."
  ([wb wb-map]
