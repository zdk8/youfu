define(function(){
    var arr_combobox = ['gender','nation','marriage','politicalstatus','positiontype','edutype','politicsstatus','technicalpost'];
    var arr_datebox = ['worktime','partytime','employtime','contractsigntime','contractdeadline'];
    var arr_validatebox = ['name','identityid'];

    var field1 = ['edutype','college','profession'];//学位学历
    var field2 = ['appellation','fm_name','fm_identityid','politicsstatus','fm_workunit','fm_position','fm_contactway'];//主要家庭成员
    ...
    var updateFunc = function (local,option) {
        ...
        for(var i=0;i<field1.length;i++){
            if(educationway[0]){
                if(local.find('[opt='+field1[i]+']').hasClass('easyui-combobox')){
                    if(educationway[0].educationtype){
                        local.find('[opt='+field1[i]+']').combobox('setValue',educationway[0].educationtype);
                    }
                }else{
                    if(educationway[0][field1[i]]){
                        local.find('[opt='+field1[i]+']').val(educationway[0][field1[i]]);
                    }
                }
            }
        }
        for(var j=0;j<field2.length;j++){
            if(familymembers[0]){
                if(local.find('[opt='+field2[j]+']').hasClass('easyui-combobox')){
                    if(familymembers[0].fm_politicalstatus){
                        local.find('[opt='+field2[j]+']').combobox('setValue',familymembers[0].fm_politicalstatus);
                    }
                }else{
                    if(familymembers[0][field2[j]]){
                        local.find('[opt='+field2[j]+']').val(familymembers[0][field2[j]]);
                    }
                }
            }
        }
        cj.initTrData(local,'edu',educationway,field1);
        cj.initTrData(local,'fam',familymembers,field2);
